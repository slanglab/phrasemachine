...
taggers availalbe in software pakcages availabe for other languages (such as 
...
However, we expect issues with Java to be the most common prolems users 
encounter when trying to install and use the `OpenNLP` package, which we use for
POS tagging. In partcular, If you are trying to install this package on a newer
...
regular expression for phrase extration, but defaults to `"(A|N)*N(PD*(A|N)*N)*"`
